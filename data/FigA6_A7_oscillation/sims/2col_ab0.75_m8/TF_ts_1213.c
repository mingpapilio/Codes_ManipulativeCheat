...
int main(){
    // Switches
        int s0= 1;                      // 0: without stacking (n= num_ini_idvl), 1: with stacking (random sampling)
        int s1= 1;                      // Including the mutation of selfishness into the model
        int s2= 1;                      // Including sneaky cheats into the model
        int s3= 1;                      // Including suppression into the model

    // General parameters
        int T= 3E3;                     // Number of generations ///////////////////////////////////
        int N_rep= 10;                  // Number of repetitions
        int num_ini_idvl= 8;            // Number of initial group members (Relatedness= its inverse)
        int n= 100;                     // Number of individual in each social group
        if(s0==0) n= num_ini_idvl;      ///////////////////////////////////
        int N_samp= 1E3;                // Number of sampling in calculating R
        int N_total= 1E5;               // Total number of individual
        int G= ceil((double)N_total/n); // Number of groups
        double mut_rate= 0.01;          // Mutation rate ///////////////////////////////////
        double MutStep= 0.1;            // Size of mutation for p, probability of being division 1
        double a= 0.75;                  // Nonlinearity parameter fo x
        double b= 0.75;                  // Nonlinearity parameter fo u
        int N_trait= 5;
    ...
    // Temporal space
        ...
        //
        double globalZ[N_rep];                          // Global P of the population
        double globalX1[N_rep];
        double globalU1[N_rep];
        double globalX2[N_rep];
        double globalU2[N_rep];
        double globalW[N_rep];
        double globalWP[N_rep];
        double globalCov[N_rep];
        double globalR[N_rep];                          // Global R of the population
        double globalR_pop[N_rep];
        double globalR_grp[N_rep];
        ...
        for(t=1; t<= T; ++t){
            for(rep=0; rep<N_rep; ++rep){
                ...
                for(i=0; i<G; ++i){
                    ...
                    for(j=0; j<n; ++j){
                        if(s0==0) rand_samp= j;
                        if(s0==1) rand_samp= floor(dsfmt_genrand_open_close(&dsfmt)*num_ini_idvl);
                        // 
